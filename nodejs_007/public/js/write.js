...
$(function () {
  $("#b_text").summernote({
    ...
    callbacks: {
       ...
       *
       * summernote 입력창에 이미지를 drag-and-drop 하면
       * 	입력창에 바로 이미지를 추가하면서 글을 작성할 수 있다.
       *
       * 이런식으로 이미지를 추가하면
       * 	작성되는 글 내용에 이미지가 encoding된 코드로 변경되고
       * 	글 내용의 크기가 어마어마하게 커져버린다.
       * DB에 해당 내용을 저장하려면
       * 	BLOB, CLOB type으로 칼럼으로 만들고 저장하는 방법이 있다.
       *
       ...
       * 	Text type이나 VARCHAR(4000) 과 같은 type 이하로만
       * 	칼럼을 설정할 것을 권장한다.
       *
       * 때문에 이미지가 포함된 데이터는
       * 실제로 서버 업로드가 안되고
       * DB Table 에도 저장할 수 없다고 생각하고 App을 개발, 설계 하는 것이 좋다.
       *

...
/**
 * promise
 * 		지금까지 사용된 JS code는 대부분이 비동기방식이고
 * 		코드(함수)들의 순서가 보장되지 않았다.
 * 		순서대로 작성된 코드가 멋대로 실생되기 때문에
 * 		그 부분을 해결하기 위해 call back 이라는 개념으로 코딩을 했다.
 *
 * 		그러나 call back 이 계속 겹치는 상황이 발생할 수 있고
 * 		그러다보면 가독성이 매우 떨어지는 코드가 된다.
 * 		a(call()=>{
 * 			b(call()=> {
 * 			})
 * 		})
 * 		JS에서 이러한 상황을 Callback Hell 이라고 부른다.
 *
 * async await 를 사용해서 해결한다.
 *
 */
router.get("/order/:table_id/input/:menu_id", async (req, res) => {
  ...
  // table_orders에서 데이터 select 하기
  const table_order = await tbl_table_orders.findOne({
    // findOnd이 완료되지않으면 이후로 진행x
  ...
  // 수량만 ++ 하여 Update를 수행하자
  if (table_order) {
    const order_qty = table_order.dataValues.to_qty; // Router에서는 dataValues를 사용해야한다.
